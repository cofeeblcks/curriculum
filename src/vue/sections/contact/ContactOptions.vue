<template>
	<div class="row pt-1 pt-lg-3">
		<div v-for="item in props.items" class="col-12 col-md-4">
			<!-- Item -->
			<div class="contact-item">
				...
				<div class="contact-item-content">
					...
					<!-- Item Link... -->
					<a v-if="item['href']" v-html="_getItemLabel(item)" :href="item['href']" class="text-4 muted" target="_blank" />

					<!-- Fallback (if there's not a link...) -->
					<p v-else v-html="_getItemLabel(item)" class="text-4 text-muted mb-0" />
				</div>
			</div>
		</div>
	</div>
