	...
	<SectionTemplate :section-data="props.sectionData">
		<div class="info-row row gx-4 gx-xl-5">
			<div v-for="subcategory in props.sectionData['content']['subcategories']" class="col-12 subcategory-col">
				<!-- Subcategory Title -->
				<SubHeading v-if="subcategory['locales']['title']" :title="subcategory['locales']['title']" :fa-icon="subcategory['faIcon']" />

				<!-- Group Component -->
				<component :is="_getComponentFor(subcategory)" :items="_fetchAndParseItemsFor(subcategory)" />
			</div>
		</div>
	</SectionTemplate>
</template>
	...
	const _fetchAndParseItemsFor = (subcategory) => {
	    ...
	    for(const item of items) {
	        ...
	        if(subcategory['progress']) {
                ...
                const breakpointsLevels = subcategory['levels']
                ...
                const parsedLevels = utils.parseLevels(percentage, breakpointsLevels)
                item['formattedLevels'] = typeof breakpointsLevels !== 'string'
	                ? data.getString(parsedLevels)
	                : parsedLevels
	        }
	    }
	    return items
